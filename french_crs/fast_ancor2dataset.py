...
class read_ancor_file():
 ...
 # return text with the related mentions 
 
    def generate_json_text_with_mentions(self):

        text_with_mentions=[]

        for n in self.root_xml.iter():

            if n.tag in ["Sync","Who","anchor"]:

                if n.text!=None:
                    if n.text.strip()!="":
                        if "id" in n.attrib and "num" in n.attrib:
                            text_with_mentions.append([n.text.strip(),n.attrib["id"]])
                        else:
                            text_with_mentions.append([n.text.strip(),""])
                            
                if n.tail!=None:        
                    if n.tail.strip()!="":
                        text_with_mentions.append([n.tail.strip(),""])

        self.json_text_with_mentions=text_with_mentions
        
        return(self.json_text_with_mentions)
